...
echo "🔨 Building Pusher client library..."
cd sockudo-js

# Build with legacy OpenSSL provider for Node v17+
echo "Building web bundle (unminified)..."
NODE_OPTIONS='--openssl-legacy-provider' MINIMIZE=false npx webpack --config=webpack/config.web.js
...
cp dist/web/pusher.js ../public/pusher-local.js
